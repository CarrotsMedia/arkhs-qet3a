...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        if args.search:
            ...
            scraper.save_json(f"elbadr_search_{args.search.replace(' ', '_')}.json")
            return

        if args.category:
            await scraper.scrape_category(page, args.category, args.pages)
            scraper.save_json(f"elbadr_cat_{args.category}.json")
            return

        if args.all:
            for cat in CATEGORIES.keys():
                await scraper.scrape_category(page, cat, args.pages)
            scraper.save_json("elbadr_all_products.json")
