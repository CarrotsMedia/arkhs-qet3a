...
def write_progress(store_slug: str, status: str, products_scraped: int, processed_count: int, total_count: int, current_keyword: str):
    try:
        progress_path = OUTPUT_DIR / f"progress_{store_slug}.json"
        data = {
            ...
            "percentage": round((processed_count / total_count) * 100) if total_count > 0 else 0,
...
def delete_progress(store_slug: str):
    try:
        progress_path = OUTPUT_DIR / f"progress_{store_slug}.json"
...
async def main():
    ...
    try:
        async with async_playwright() as pw:
            ...
            if args.search:
                write_progress("amazon", "running", 0, 0, 1, args.search)
                ...
                write_progress("amazon", "running", len(scraper.products), 1, 1, args.search)
                
            elif args.mobiles:
                keywords = ["smartphone", "mobile phone", "tablet"]
                total = len(keywords)
                for idx, keyword in enumerate(keywords):
                    write_progress("amazon", "running", len(scraper.products), idx, total, keyword)
                    await scraper.search_products(page, keyword)
                    await asyncio.sleep(3)
                scraper.save_json("amazon_mobiles.json")
                write_progress("amazon", "running", len(scraper.products), total, total, "Completed")
